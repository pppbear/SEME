...
def prepare_data(df, target_var, selected_features=None):
    ...
    print(f"X 中的 NaN 数量: {np.isnan(X).sum()}")
    print(f"X 中的 Inf 数量: {np.isinf(X).sum()}")
    print(f"X 的均值: {np.mean(X):.4f}")
    print(f"X 的标准差: {np.std(X):.4f}")
    print(f"X 的最大值: {np.max(X):.4f}, 最小值: {np.min(X):.4f}")
    print(f"y 的均值: {np.mean(y):.4f}")
    print(f"y 的标准差: {np.std(y):.4f}")
    print(f"y 的最大值: {np.max(y):.4f}, 最小值: {np.min(y):.4f}")
    ...
    if constant_features:
        print(f"警告：以下特征为常数特征（归一化后所有值相同）: {constant_features}")
    if zero_features:
        print(f"警告：以下特征全为0: {zero_features}")
    
    # 检查input_dim和X.shape
    print(f"input_dim（特征数）: {len(feature_cols)}，X.shape[1]: {X.shape[1]}")
    if len(feature_cols) != X.shape[1]:
        print("警告：input_dim与X.shape[1]不一致！")
    if X.shape[1] == 0:
        print("警告：没有可用特征，X.shape[1]为0！")
    
    # 检查y是否有方差
    y_var = np.var(y)
    print(f"y 方差: {y_var:.8f}")
    if y_var == 0:
        print("警告：目标变量y无方差，全为常数！")
    ...
    # 保存标准化后的数据到Excel（只保存全部数据，便于溯源）
    normalized_df = pd.DataFrame(X, columns=feature_cols)
    normalized_df[target_var] = y
    for col in feature_cols:
        normalized_df[f'{col}_original'] = df_filtered[col].values
    normalized_df[f'{target_var}_original'] = df_filtered[target_var].values
    output_file = f'simplified_results/normalized_data_{target_var}.xlsx'
    ...
    # 自动排查和修复nan/inf
    for name, arr in zip(['X_train', 'X_test', 'y_train', 'y_test'], [X_train_tensor, X_test_tensor, y_train_tensor, y_test_tensor]):
        n_nan = torch.isnan(arr).sum().item()
        n_inf = torch.isinf(arr).sum().item()
        if n_nan > 0 or n_inf > 0:
            print(f"警告：{name} 中存在 {n_nan} 个NaN和 {n_inf} 个Inf，已自动用0填充！")
            arr = torch.nan_to_num(arr, nan=0.0, posinf=0.0, neginf=0.0)
        # 更新变量
        if name == 'X_train': X_train_tensor = arr
        if name == 'X_test': X_test_tensor = arr
        if name == 'y_train': y_train_tensor = arr
        if name == 'y_test': y_test_tensor = arr
    ...
    for k, v in dataset.items():
        print(f"{k} NaN: {torch.isnan(v).sum().item()}, Inf: {torch.isinf(v).sum().item()}")
...
def predict_with_kan(file_path, target_var, test_data=None):
    ...
    # 保存特征列表
    with open(f'simplified_results/models/{target_var}_features.txt', 'w') as f:
